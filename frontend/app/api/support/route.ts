...
/**
 * System prompt for the support intake assistant
 * This prompt is injected at the start of every conversation to guide the AI's behavior
 */
const SYSTEM_PROMPT = `You are a helpful and friendly customer support assistant for Barfield, an aviation maintenance provider.
...
1. Conversation:
   ...
   - Try to get the "part_reference" (part number or ID) if the user knows it.
...
/**
 * Knowledge base documents injected as context
 * These documents help the AI determine the correct team and provide better recommendations
 */
const DOCUMENTS = `# Knowledge Base Documents
...
export async function POST(req: Request) {
  try {
    ...
    const result = streamText({
      // @ts-expect-error - conflict with zod
      model: openai("gpt-4o"),
